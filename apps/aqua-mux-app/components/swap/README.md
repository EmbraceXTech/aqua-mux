...
`/swap` and `/swap/design` both render `components/views/SwapView.tsx`. Neither route nor the production view imports the `swap-design` folder. The old design files can be deleted independently.

## Component ownership

- `SwapView` composes the layout, wallet header, composer, and dialogs.
- `useSwapWorkspace` owns draft and dialog transitions. Its pure `lib/swap-draft.ts` reducer keeps mode drafts independent and applies immutable row edits. `useLiveSwap` owns quotes, balances, and wallet execution.
- `SwapComposer` composes `SwapModeControls`, `SwapTokenSection`, `SwapStatus`, and `SwapActions`. Token sections render controlled `SwapTokenRow` components defined at module scope.
- `SwapDialogs` keeps one accessible modal shell mounted. Its discriminated dialog state allows only one overlay at a time. Token search belongs to the picker session; slippage is controlled by the workspace. Review content and transaction actions are separate from the review summary.
- Dialogs restore focus explicitly to the opener, including the same field slot after replacing a keyed token row. All styling belongs to `components/swap/swap.module.css`.

Legacy `swap-output-*`, `swap-details`, and `swap-flow-preview` components are not used by this view.
...
The old `components/swap-design/model.ts` and its tests contain the original design fixtures. The live component does not import them. It does not display sample balances or fabricated USD prices.
...
```sh
npx tsx --test test/live-swap.test.ts test/swap-transaction-status.test.ts test/swap-draft.test.ts
npx tsx scripts/verify-live-swap-fork.ts
# With the app running at http://127.0.0.1:3101:
npx tsx scripts/test-live-swap-ui.ts
SWAP_TEST_PATH=/swap/design npx tsx scripts/test-live-swap-ui.ts
```
...
The UI script launches an isolated browser with a synthetic wallet and mocked quote API. It checks picker focus restoration, search reset, controlled slippage, rejection, both modes and exact sides, allocation errors, approval review, confirmed/reverted receipts, unlocated hashes, mobile overflow, and runtime errors. These UI tests do not establish mainnet execution success.
