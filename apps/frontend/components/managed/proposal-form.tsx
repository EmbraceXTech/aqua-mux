...
export function ProposalForm({
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (
        !/^\d+(\.\d+)?$/.test(budget) ||
        parseUnits(budget, funding.decimals) <= 0n
      )
      ...
      await onSubmit({
        ...
        budget: parseUnits(budget, funding.decimals).toString(),
        gasReserveWei: parseUnits(reserve, 18).toString(),
        permittedAssets: [
          ...new Set([
            funding.address,
  ...
  return (
    <form onSubmit={submit} className="managed-stack">
      ...
      <div className="managed-layout">
        ...
        <aside className="managed-panel managed-stack">
          ...
          <Button type="submit" disabled={busy || !account}>
            {busy ? "Analyzing current wallet..." : "Generate fresh proposal"}
          </Button>
        </aside>
      </div>
      {error && (
