...
import attestation from "../lib/server/route-policy/fixtures/provenance/attestation.json";
import calls from "../lib/server/route-policy/fixtures/bnb-calls.json";
import {
...
test("retained compiler inputs and semantic immutables reconstruct every enabled deployment", () => {
  for (const build of Object.values(attestation.builds)) {
    const input = readFileSync(
      new URL(
        `../lib/server/route-policy/fixtures/provenance/${build.inputFile}`,
        import.meta.url,
      ),
    );
    assert.equal(
  ...
  for (const record of attestation.deployments) {
    ...
    const pool = poolDeployments.find((p) => p.id === record.role);
...
test("real BNB calldata fixtures enforce observed output minima", () => {
  for (const record of calls) {
    const route = record.route as VerifiedRoute;
    validateCompiledRoute(route.request, route, route.quotedAt + 1);
