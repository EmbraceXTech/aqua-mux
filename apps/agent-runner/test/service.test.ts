...
async function fixture(
  t: test.TestContext,
  provider: ProviderReview = async () => stubReview(),
...
test("HTTP authenticates, rejects unsafe endpoints/payloads, and returns validated persisted result", async (t) => {
  ...
  assert.equal(
    (await f.post(undefined, { authorization: "Bearer wrong" })).status,
    401,
  );
  assert.equal(
    (await f.post(undefined, { origin: "https://evil.invalid" })).status,
    403,
  );
  assert.equal(
    (await f.post(undefined, { "idempotency-key": "wrong" })).status,
    400,
  );
  assert.equal(
    (await f.post(undefined, { "content-type": "text/plain" })).status,
    415,
  );
  const auth = { authorization: `Bearer ${f.token}` };
  assert.equal(
    (await fetch(`${f.url}/shell`, { method: "POST", headers: auth })).status,
    404,
  );
  assert.equal(
    (
      await fetch(`${f.url}/reviews`, {
        method: "POST",
...
test("freshness, identity, and model policy or raw transaction mutations fail closed", async (t) => {
  ...
  stale.snapshot.blockTimestamp = 0;
  assert.equal((await f.post(stale)).status, 422);
  const foreign = requestFixture();
  foreign.snapshot.maker = "0x0000000000000000000000000000000000000099";
  assert.equal((await f.post(foreign)).status, 400);
  const request = requestFixture();
  assert.throws(
    () =>
      validateResult({ ...stubReview().result, calldata: "0x1234" }, request),
    /invalid_review_result/,
  );
  const config = structuredClone(request.config!);
  config.policy.maxActions.value++;
  assert.throws(
    () =>
      validateResult(
        { ...stubReview().result, proposedConfig: config },
        request,
      ),
    /model_policy_mutation/,
  );
  assert.throws(
    () => parseRequest({ ...request, shell: "anything" }, request.requestId),
    /invalid_request/,
  );
});
...
test("cancellation aborts active inference and durable retry never restarts it", async (t) => {
  ...
  await new Promise((resolve) => setTimeout(resolve, 30));
  const response = await fetch(`${f.url}/reviews/${request.requestId}`, {
    method: "DELETE",
...
test("SQLite restart preserves completed usage and terminally interrupts unfinished requests", async (t) => {
  ...
  store.finish(completed.requestId, "succeeded", {
    ...stubReview(),
    requestId: completed.requestId,
  });
  store.close();
  const restarted = new ReviewStore(path);
  const reviews = new Reviews(restarted, async () => {
    assert.fail("Restart must not invoke inference");
  });
