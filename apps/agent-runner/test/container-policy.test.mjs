...
const identity = { id: 'aquamux-runtime-abc', imageId: 'sha256:abc', owner: 'fixture', instanceId: 'b'.repeat(64) };
const baseline = {
  Id: identity.instanceId, Name: '/aquamux-runtime-abc', Image: identity.imageId, State: { Running: true }, Mounts: [],
...
test('resume rejects changed identity and missing creation restrictions', () => {
  ...
  const mutations = [
    ...
    x => x.HostConfig.SecurityOpt = [], x => x.HostConfig.SecurityOpt.push('seccomp=unconfined'),
    x => x.HostConfig.UTSMode = 'host', x => x.HostConfig.UsernsMode = 'host', x => x.HostConfig.CgroupnsMode = 'host', x => x.HostConfig.Privileged = true,
    ...
    x => x.Id = 'different-container', x => x.Image = 'different-image', x => x.Config.Labels['aquamux.runtime-owner'] = 'different-owner',
