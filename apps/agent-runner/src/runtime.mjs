...
export function createAgent(provider, { structured = false } = {}) {
  ...
  return new HarnessAgent({
    ...
    instructions: 'This is an isolated runtime compatibility test. Follow the exact requested response. Never inspect credentials or use tools unless the test explicitly asks for a harmless command.',
    ...(structured ? { output: Output.object({ schema: jsonSchema(reviewSchema) }) } : {}),
