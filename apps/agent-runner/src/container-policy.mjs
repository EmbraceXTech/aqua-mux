...
export function validateContainer(info, identity) {
  const config = info.HostConfig;
  const bindings = info.NetworkSettings.Ports;
  const restrictionsHold = info.Name === `/${identity.id}`
    ...
    && config.CapDrop?.includes('ALL') && !config.CapAdd?.length
    ...
    && !config.PidMode && ['', 'private'].includes(config.IpcMode)
...
export async function acquireContainer(execute, { identity, image = DEFAULT_IMAGE, abortSignal } = {}) {
  let owned = false;
  ...
  try {
    if (!selected) {
      ...
      owned = true;
      await execute(containerArguments(selected.id, selected.imageId, selected.owner), undefined, { abortSignal, timeoutMs: 30000 });
    ...
    if (!/^aquamux-runtime-[a-f0-9-]+$/.test(selected.id)
      ...
      || !/^[a-f0-9-]{36}$/.test(selected.owner)) throw new DockerOperationError('sandbox-identity-invalid');
    const info = JSON.parse((await execute(['inspect', selected.id], undefined, { abortSignal })).toString())[0];
    validateContainer(info, selected);
    return { identity: selected, binding: info.NetworkSettings.Ports['4000/tcp'][0] };
  } catch (error) {
    if (owned) {
      try { await execute(['rm', '-f', selected.id], undefined, { timeoutMs: 10000 }); }
