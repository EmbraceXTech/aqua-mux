...
export const defaultLimits: ReviewLimits = {
  concurrency: 2,
  timeoutMs: 120_000,
  reviewsPerMinute: 6,
  reviewsPerDay: 200,
...
export class Reviews {
  private active = new Map<
    string,
    { controller: AbortController; promise: Promise<ReviewResponse> }
  >();
  ...
  constructor(
    ...
    private provider: ProviderReview,
    ...
    if (this.limits.concurrency > 8 || this.limits.timeoutMs > 300_000)
      throw new Error("Review limits exceed safety ceiling");
  }
  ...
  async submit(request: ReviewRequest): Promise<ReviewResponse> {
    ...
    if (this.store.resourceRequests().some((id) => !this.active.has(id)))
      throw new ServiceError(503, "resource_cleanup_required");
    if (this.active.size >= this.limits.concurrency)
      throw new ServiceError(429, "concurrency_exhausted");
    const now = Date.now();
    if (
      this.store.countSince(now - 60_000) >= this.limits.reviewsPerMinute ||
      this.store.countSince(now - 86_400_000) >= this.limits.reviewsPerDay
    )
  ...
  private restore(row: StoredReview): ReviewResponse {
    ...
    throw new ServiceError(
      row.error_status ?? 409,
      row.error_code ?? "review_not_available",
    );
  }
  ...
  private async run(
    ...
    const timeout = Math.min(
      ...
      (request.deadline ?? Infinity) - Date.now(),
    );
    const timer = setTimeout(
      () => controller.abort(new ServiceError(504, "review_timeout")),
      Math.max(1, timeout),
    );
    try {
      ...
      controller.signal.throwIfAborted();
      const response = validateResponse(output, request);
      ...
      this.store.finish(
        request.requestId,
        failure.code === "review_cancelled" ? "cancelled" : "failed",
        null,
        failure.code,
        failure.status,
      );
  ...
  cancel(id: string): { requestId: string; status: string } {
    ...
    this.active
      .get(id)
      ?.controller.abort(new ServiceError(409, "review_cancelled"));
