...
export function validateServerToken(token: string): void {
  ...
  if (!/^[A-Za-z0-9_+/=-]{43,256}$/.test(token) || new Set(token).size < 16)
    throw new Error(
      "AQUAMUX_AGENT_RUNNER_TOKEN must be a randomly generated token of at least 32 bytes",
    );
}
...
export function createReviewServer(
  reviews: Reviews,
  token: string,
  options: { maxPayloadBytes?: number; requestsPerMinute?: number } = {},
) {
  ...
  const server = createServer(async (req, res) => {
    try {
      const auth = createHash("sha256")
        ...
        .digest();
      if (!timingSafeEqual(expected, auth))
      ...
      if (req.method !== "DELETE" && ++requests > requestsPerMinute)
        throw new ServiceError(429, "request_rate_exceeded");
      if (req.headers.origin)
        throw new ServiceError(403, "browser_origin_forbidden");
      if (req.method === "POST" && req.url === "/reviews") {
        ...
        req.method === "DELETE" &&
        /^\/reviews\/[a-zA-Z0-9:_-]{1,160}$/.test(req.url ?? "")
      ) {
